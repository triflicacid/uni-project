...
namespace language::statement {
  class FunctionRef : public Expression {
    ...
    public:
    FunctionRef(int pos, const lexer::Token *token, std::string name)
        : Expression(pos, token), m_name(std::move(name)), m_func(nullptr) {};
    ...
    [[nodiscard]] const types::Type *get_type_of() const override {
      return m_func == nullptr ? nullptr : m_func->function_type();
    }
