...
uint64_t bus_load(const bus_t *bus, uint64_t addr, uint8_t size) {
  ...
  printf(DEBUG_STR ANSI_YELLOW " bus load" ANSI_RESET ": %d bytes from address 0x%llx = 0x%llx\n", size, addr, value);
  return value;
#else
...
#if DEBUG & DEBUG_DRAM
  printf(DEBUG_STR ANSI_YELLOW " bus store" ANSI_RESET ": value 0x%llx of %d bytes to address 0x%llx\n", value, size,
         addr);
#endif
