...
%    \end{center}
    ...
    \begin{longtable}{|c|c|l|l|l|}
        ...
        \texttt{\$r2} = max length} & \makecell[l]{Read a null-terminated string into \texttt{\$r1}.\\%
        String is truncated to maximum length, \texttt{\$r2}.} & \textit{None} \\
        \hline \hline
        ...
        \hline
        mem\_copy & 12 & \makecell[l]{\texttt{\$r1} = source address\\%
        \texttt{\$r2} = destination address\\%
        \texttt{\$r3} = length in bytes} & \makecell[l]{Copy \(n\) bytes from one region to another.\\%
