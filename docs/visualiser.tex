...
$ ./visualiser --asm source.s --bin source
    ...
    \begin{itemize}
        \item{[} and \texttt{]} -- both change the file and line selection by tracing the selected line forwards or backwards, respectively.
        That is, is a source (\texttt{.s}) line is selected, \texttt{[} will select the line in an assembly (\texttt{.asm}) source, whereas \texttt{]} does nothing.
