...
maths + 1; // error: expected rvalue, got namespace
    ...
    For example, \texttt{u8} may be coerced into \texttt{u64}, or \texttt{float} into \txettt{double}, but \textbf{not} \texttt{i32} into \texttt{i16}.
...
let a = 42 as u8; // i32 -> u8
    ...
    \begin{table}[h]
        ...
        \begin{tabular}{|l|l|c|}
            ...
            \hline
            \texttt{(T) a} & Primitive cast & \multirow{5}{*}{\(\longleftarrow\)} \\
            \texttt{-a} & Negation & \\
            \texttt{~a} & Bitwise NOT & \\
            \texttt{!a} & Boolean NOT & \\
            \texttt{registerof a} & Register lookup & \\
            \hline
            \texttt{a as T} & Cast & \(\longleftarrow\) \\
            \hline
