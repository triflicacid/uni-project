...
namespace language::statement {
    Function::Function(std::string  name, types::FunctionType *type, int id)
            : Statement(type->position()), m_name(std::move(name)), m_type(type), m_body(nullptr), m_id(id), m_is_complete(false), m_stack_size(0) {
        type->set_id(id);
    };
    ...
    std::string Function::to_label() const {
        std::stringstream stream;
        stream << "f_" << m_name;

        if (!m_params.empty()) {
            for (int i = 0; i < m_type->argc(); i++) {
                stream << '_' << m_type->arg(i)->repr();
            }
        }
