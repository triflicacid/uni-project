...
namespace language::lexer {
  ...
  void Lexer::generate_bracket_mismatch_error(lexer::Token &open_bracket, lexer::Token &closing_bracket,
                                              message::List &list) {
    std::stringstream stream;
    stream << "Bracket mismatch: expected " << lexer::Token::repr((lexer::Token::Type) (open_bracket.type() + 1))
           << " to close " << lexer::Token::repr(open_bracket.type()) << ", got "
           << lexer::Token::repr(closing_bracket.type()) << ".";
    ...
    while (exists()) {
      if (consume_newline()) {
        if (m_pos != line_start_at && !m_source->tokens.empty()) {
          ...
          if (!(open_bracket_paren_count > 0 || lexer::Token::is_operator(type) ||
                lexer::Token::is_opening_bracket(type))) {
      ...
      // Numeric literal?
      if (std::isdigit(peek()) || (peek() == '-' && exists(1) && std::isdigit(peek(1)))) {
        int i = index(), j = i;
        uint64_t value;
        bool is_double;
        bool ok = parse_number(*string(), j, value, is_double);

        // did we get anywhere?
        if (ok && j > i) {
          j -= i;
          add_token(Token::Type::NUMBER, extract(j));
          move(j);
          continue;
        }
      }
