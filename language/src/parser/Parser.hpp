...
namespace language::parser {
  class Parser {
    ...
    bool
    check_can_create_overload(const std::string &name, const types::FunctionType *overload, message::List &messages);
    ...
    /** Parse a number, return success. */
    bool parse_number(statement::Number **number, message::List &messages);
    ...
    message::MessageWithSource *generate_error(const std::string &message) {
      return generate_message(message::Error, message);
    }
    ...
    generate_syntax_error_multi(const std::vector<const std::vector<lexer::Token::Type> *> &expected);
    ...
    explicit Parser(Program *program) : m_prog(program), m_pos(0), m_end_pos(-1), m_tokens(program->source()->tokens),
                                        m_entry_pos(-1), m_entry_type(nullptr) {
