...
namespace lang::type {
  ...
  public:
    ...
    /** @brief Return this node, since it is already a FloatNode. */
    const FloatNode* get_float() const override { return this; }

    /** @brief Test whether this is the 64-bit double-precision type. */
    bool is_double() const { return double_; }

    /** @brief Return the node kind name, "f32" or "f64". */
    ...
    /** @brief Return 8 for double-precision, 4 for single-precision. */
    ...
    /** @brief Return the assembly datatype tag matching this type's precision. */
    constants::inst::datatype::dt get_asm_datatype() const override;

    /** @brief Always false: floats are stored by value, not referenced like a pointer. */
